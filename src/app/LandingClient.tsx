...
import { Zap, Globe, Activity, Layers, Lock } from "lucide-react"
...
export function LandingClient() {
  ...
  useEffect(() => {
    ...
    return () => {
      unsubDisaster(); unsubAnalysis(); unsubFeed();
    }
  }, [])
      ...
      <div className="relative z-10 flex flex-col h-full pointer-events-none">
        ...
        <main className="flex-1 flex flex-col items-center justify-center text-center px-6 gap-8">
          <div className="space-y-2 animate-in fade-in zoom-in duration-1000">
            <h1 className="text-[120px] font-black leading-none tracking-tighter italic text-white drop-shadow-[0_0_30px_rgba(255,255,255,0.3)]">TERRA</h1>
            <p className="text-xl font-medium tracking-[0.4em] uppercase text-primary/80">Disaster Intelligence • Real-Time • AI-Powered</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full max-w-4xl pointer-events-auto">
            ...
            <div className="glass p-6 rounded-2xl border-white/10 backdrop-blur-2xl flex flex-col items-center gap-2">
              ...
              <div className="text-3xl font-black text-white">{analysis?.confidence ? `${analysis.confidence}%` : "96.4%"}</div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 pt-8 pointer-events-auto">
            <Link href="/dashboard">
              <Button className="h-16 px-12 text-lg font-black uppercase tracking-widest bg-gradient-to-r from-destructive to-orange-600 hover:scale-105 transition-transform shadow-2xl shadow-destructive/20 border-b-4 border-black/20">Enter Command Center</Button>
            </Link>
            <Link href="/citizen">
              <Button variant="outline" className="h-16 px-12 text-lg font-black uppercase tracking-widest border-2 border-white/20 bg-white/5 hover:bg-white/10 backdrop-blur-xl text-white">Citizen Safety Portal</Button>
            </Link>
          </div>
        </main>
          ...
          <div className="flex whitespace-nowrap animate-ticker py-2">
            {[...tickerMessages, ...tickerMessages].map((msg, i) => (
              <div key={i} className="flex items-center gap-4 px-8 border-r border-white/5 group hover:bg-white/5 transition-colors cursor-default">
                <span className={cn("text-lg", msg.priority === 'CRITICAL' ? 'text-destructive' : msg.priority === 'WARNING' ? 'text-amber-500' : 'text-primary')}>●</span>
                <span className={cn("text-[10px] font-bold uppercase tracking-widest transition-colors", msg.priority === 'CRITICAL' ? 'text-destructive' : msg.priority === 'WARNING' ? 'text-amber-500' : 'text-white/80 group-hover:text-white')}>
                  {msg.priority}: {msg.message} — {new Date(msg.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                </span>
              </div>
            ))}
            {tickerMessages.length === 0 && (
              <div className="px-8 text-[10px] font-bold uppercase tracking-widest text-muted-foreground italic">● Establishing neural link to global tactical nodes... satellite sync in progress...</div>
            )}
          </div>
        </footer>
      </div>
